...
public class ScoreboardWrapper {
    ...
    public String getPlayerScoreboardLine(Player player, int index) {
        Scoreboard scoreboard = player.getScoreboard();
        if (scoreboard == null || scoreboard.getObjective(DisplaySlot.SIDEBAR) == null) {
            throw new IllegalStateException("The player does not have a scoreboard with a sidebar objective.");
        }

        Objective objective = scoreboard.getObjective(DisplaySlot.SIDEBAR);
        List<Score> scores = new ArrayList<>(scoreboard.getScores(objective.getDisplayName()));

        // Sort the scores based on the score value to maintain the order
        scores.sort((score1, score2) -> Integer.compare(score2.getScore(), score1.getScore()));

        if (index < 0 || index >= scores.size()) {
            throw new IndexOutOfBoundsException("The index cannot be negative or higher than the number of lines.");
        }

        return scores.get(index).getEntry();
    }
    ...
    public void addLine(String line) {
        ...
        objective.getScore(modified).setScore(MAX_LINES - modifies.size());
    }
